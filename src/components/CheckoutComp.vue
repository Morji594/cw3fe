    ...
    <div class="checkout">
      <div class="section1">
        <div v-for="lesson in cart" :key="lesson.id" class="list">
          ...
          <button class="btn" @click="removeCourseFromCart(lesson)">
            Remove
          </button>
        </div>
      </div>
      <div class="submit">
        ...
        <input
          ...
          name="phone-number"
          id="phone-number"
          v-model="checkout.Phone_No"
        />
        <button type="submit" class="submitBtn" id="btnSub" @click="isNumber">
          Checkout
        </button>
      </div>
    </div>
  ...
  methods: {
    async getData() {
      try {
        const response = await axios.get(
          "https://courseworktwoserver.herokuapp.com/collection/Orders/61fbfba1d7f5ca516558514e"
        );
    ...
    async isNumber() {
      ...
      if (phoneNumber.value.match(integers) && name.value.match(letters)) {
        ...
        alert(
          "Please input only numeric characters for Phone number and letters for Name"
        );
...
.container {
  ...
  .header {
    ...
    .sort {
      select {
        padding: 10px 10px;
        border-radius: 5px;
        border: 1px solid rgba(241, 240, 240, 0.15);
  ...
  .section1 {
    ...
    .btn {
      ...
      margin: auto;
      padding: 10px 20px;
      background-color: rgba(32, 114, 221, 0.9);
      color: white;
      border-radius: 10px;
    }
    .btn:hover {
